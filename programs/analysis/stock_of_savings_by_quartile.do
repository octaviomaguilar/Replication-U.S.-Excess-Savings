...
This code will create figure 7a from the FEDS note: https://www.federalreserve.gov/econres/notes/feds-notes/excess-savings-during-the-covid-19-pandemic-20221021.html
...
3.) The baseline period for the excess savings is 2020q1 - 2022q2. 

The baseline parameters will yield results that match figure 7a in Aditya Aladangady, David Cho, Laura Feiveson, and Eugenio Pinto (2022)
...
merge 1:1 yq using "$data/time_varying_shares.dta", keep(3) nogen
*not matched are periods before 2020q1.

/* Figure 7a from FEDS note */ 
* 2.2: Define income quartile shares: The shares are time varying and are allocated based on the shares reported on the FEDS note. 
...
save "$data/output/excess_savings_quartiles.dta", replace
export excel "$data/output/excess_savings_quartiles.xlsx", firstrow(variables) replace
